...
const questions: Omit<SeedQuestion, 'id'>[] = [
  // ─── Géographie ────────────────────────────────────────────
 ...
 */
export function seedQuestions(db: Database.Database): void {
  const count = (db.prepare('SELECT COUNT(*) as count FROM questions').get() as { count: number }).count;

  if (count > 0) {
    logger.info(`Seed ignoré — ${count} questions déjà présentes`);
    return;
  }
  ...
  const insertMany = db.transaction((qs: SeedQuestion[]) => {
    for (const q of qs) {
      insert.run(q);
    }
  });

  const withIds: SeedQuestion[] = questions.map(q => ({ ...q, id: uuidv4() }));
  insertMany(withIds);

  logger.info(`Seed terminé — ${withIds.length} questions insérées`);
}
