...
exports.handleChooseMode = handleChooseMode;
exports.handleSubmitQuestion = handleSubmitQuestion;
exports.handleValidateQuestion = handleValidateQuestion;
...
exports.handleValidateAnswer = handleValidateAnswer;
exports.handleUseTimeJoker = handleUseTimeJoker;
exports.handleUseHintJoker = handleUseHintJoker;
const constants_1 = require("@shared/constants");
const logger_1 = require("../logger/logger");
const RoomManager_1 = require("./RoomManager");
const questionRepository_1 = require("../questions/questionRepository");
const TimerManager_1 = require("./TimerManager");
const uuid_1 = require("uuid");
// Map globale des états de match actifs indexés par roomCode
const activeMatches = new Map();
function getMatchState(roomCode) {
    return activeMatches.get(roomCode) ?? null;
}
/**
 * Initialise et démarre un match spécifique du bracket.
 */
function startMatch(io, roomCode, matchId) {
    const roomState = (0, RoomManager_1.getRoomByCode)(roomCode);
    if (!roomState || !roomState.bracket) {
        logger_1.logger.error('Impossible de démarrer le match : bracket introuvable', { roomCode, matchId });
        return;
    }
    // Trouver le match correspondant dans le bracket
    let match = null;
    if (roomState.bracket.semiFinal1.id === matchId)
        match = roomState.bracket.semiFinal1;
    else if (roomState.bracket.semiFinal2.id === matchId)
        match = roomState.bracket.semiFinal2;
    else if (roomState.bracket.final && roomState.bracket.final.id === matchId)
        match = roomState.bracket.final;
    if (!match) {
        logger_1.logger.error('Match non trouvé dans le bracket', { roomCode, matchId });
        return;
    }
    ...
    logger_1.logger.info('Match démarré et notifié', {
        roomCode,
        matchId,
        phase: match.phase,
 ...
 */
function handleChooseMode(io, roomCode, mode) {
    ...
    logger_1.logger.info('Mode choisi pour le match', {
        roomCode,
 ...
 */
function handleSubmitQuestion(io, roomCode, text, category) {
    const roomState = (0, RoomManager_1.getRoomByCode)(roomCode);
    const matchState = activeMatches.get(roomCode);
    if (!roomState || !matchState)
        return;
    if (roomState.room.questionMode === 'official') {
        // Mode officiel : Sélection aléatoire en base de données
        const questionDb = (0, questionRepository_1.getRandomByCategory)(category);
        if (!questionDb) {
            logger_1.logger.error('Aucune question trouvée en base pour la catégorie', { category });
    ...
    logger_1.logger.info('Question soumise au Juge', {
        roomCode,
 ...
 */
function handleValidateQuestion(io, roomCode, approved) {
    const matchState = activeMatches.get(roomCode);
    if (!matchState || !matchState.currentQuestion)
        return;
    if (!approved) {
        ...
        logger_1.logger.info('Question refusée par le Juge, nouvelle demande envoyée', { roomCode });
        return;
    }
    // Question approuvée !
    ...
    const safeQuestion = {
        ...
        text: matchState.currentQuestion.text,
        category: matchState.currentQuestion.category,
        mode: matchState.currentQuestion.mode,
    };
    ...
    if (matchState.currentMode === 'discussion') {
        ...
        matchState.attemptsLeft = constants_1.GAME_CONSTANTS.MAX_ATTEMPTS;
        logger_1.logger.info('Démarrage phase réponse Discussion', {
            roomCode,
            answeringTeam: matchState.answeringTeam,
            duration: constants_1.GAME_CONSTANTS.DISCUSSION_TIMER,
        });
        (0, TimerManager_1.startTimer)(io, roomCode, constants_1.GAME_CONSTANTS.DISCUSSION_TIMER, 'discussion', () => handleDiscussionTimeout(io, roomCode));
    }
        ...
        logger_1.logger.info('Démarrage phase Enchères', {
            roomCode,
 ...
 */
function startAuctionBidTurn(io, roomCode) {
    const matchState = activeMatches.get(roomCode);
    if (!matchState || !matchState.currentBidder)
        return;
    io.to(roomCode).emit('auction:bidTurn', matchState.currentBidder, constants_1.GAME_CONSTANTS.BID_TIMER);
    (0, TimerManager_1.startTimer)(io, roomCode, constants_1.GAME_CONSTANTS.BID_TIMER, 'bid', () => handleBidTimeout(io, roomCode));
}
/**
 * Une équipe soumet une enchère numérique.
 */
function handleAuctionBid(io, roomCode, socketId, amount) {
    const matchState = activeMatches.get(roomCode);
    const roomState = (0, RoomManager_1.getRoomByCode)(roomCode);
    if (!matchState || !roomState)
        return;
    const player = roomState.players.get(socketId);
    if (!player || player.teamColor !== matchState.currentBidder) {
        logger_1.logger.warn('Tentative d\'enchère hors tour ou invalide', { socketId, roomCode });
        return;
    }
    if (amount <= matchState.highestBid) {
        logger_1.logger.warn('Montant d\'enchère trop faible', { roomCode, amount, currentHighest: matchState.highestBid });
        return;
    }
    // Arrête le timer de tour
    (0, TimerManager_1.stopTimer)(roomCode);
    ...
    io.to(roomCode).emit('auction:bidUpdate', matchState.currentBidder, amount);
    ...
    logger_1.logger.info('Enchère enregistrée', { roomCode, team: player.teamColor, amount });
 ...
 */
function handleAuctionPass(io, roomCode, socketId) {
    const matchState = activeMatches.get(roomCode);
    const roomState = (0, RoomManager_1.getRoomByCode)(roomCode);
    if (!matchState || !roomState)
        return;
 ...
 */
function handlePassLogic(io, roomCode) {
    const matchState = activeMatches.get(roomCode);
    if (!matchState)
        return;
    matchState.consecutivePasses++;
    logger_1.logger.info('Équipe passe son tour d\'enchère', {
        roomCode,
 ...
 */
function endAuctionBidding(io, roomCode) {
    const matchState = activeMatches.get(roomCode);
    if (!matchState || !matchState.highestBidder)
        return;
    ...
    logger_1.logger.info('Enchères terminées, début des réponses', {
        roomCode,
        winner,
        targetAnswersNeeded: target,
    });
    // Démarre le chrono de 30s pour fournir les réponses
    matchState.auctionRemainingTime = constants_1.GAME_CONSTANTS.AUCTION_TIMER;
    (0, TimerManager_1.startTimer)(io, roomCode, constants_1.GAME_CONSTANTS.AUCTION_TIMER, 'auction', () => handleAuctionTimeout(io, roomCode));
}
/**
 * Expiration du timer d'enchère de tour (5s).
 * Équivaut à un pass automatique.
 */
function handleBidTimeout(io, roomCode) {
    logger_1.logger.info('Timeout enchère de tour (5s)', { roomCode });
    handlePassLogic(io, roomCode);
}
/**
 * Soumission de réponse par un joueur de l'équipe qui a la main.
 */
function handleSubmitAnswer(io, roomCode, socketId, answer) {
    const matchState = activeMatches.get(roomCode);
    const roomState = (0, RoomManager_1.getRoomByCode)(roomCode);
    if (!matchState || !roomState)
        return;
    ...
    if (!player || player.teamColor !== matchState.answeringTeam) {
        logger_1.logger.warn('Tentative de réponse par un joueur non autorisé', { socketId, roomCode });
        return;
    }
    ...
    logger_1.logger.info('Réponse soumise, en attente de validation du Juge', {
        roomCode,
 ...
 */
function handleValidateAnswer(io, roomCode, approved) {
    const matchState = activeMatches.get(roomCode);
    if (!matchState || !matchState.answeringTeam)
        return;
    if (matchState.currentMode === 'discussion') {
 ...
 */
function handleDiscussionAnswerValidation(io, roomCode, approved) {
    const matchState = activeMatches.get(roomCode);
    if (!matchState || !matchState.answeringTeam)
        return;
    if (approved) {
        ...
        if (matchState.answeringTeam === matchState.activeTeam) {
            ...
            matchState.attemptsLeft = constants_1.GAME_CONSTANTS.MAX_ATTEMPTS;
            logger_1.logger.info('Équipe active correcte. L\'adverse doit répondre à la même question.', {
                roomCode,
                adverseTeam,
            });
            ...
            logger_1.logger.info('Égalité sur la manche (deux équipes correctes)', { roomCode });
        ...
        if (matchState.attemptsLeft > 0) {
            ...
            logger_1.logger.info('Réponse incorrecte, relance pour tentative restante', { roomCode, attemptsLeft: matchState.attemptsLeft });
            (0, TimerManager_1.startTimer)(io, roomCode, constants_1.GAME_CONSTANTS.DISCUSSION_TIMER, 'discussion', () => handleDiscussionTimeout(io, roomCode));
        }
        else {
 ...
 */
function handleAuctionAnswerValidation(io, roomCode, approved) {
    const matchState = activeMatches.get(roomCode);
    if (!matchState || !matchState.answeringTeam)
        return;
    if (approved) {
        ...
        io.to(roomCode).emit('match:answerValidated', true);
        logger_1.logger.info('Réponse validée positivement en Enchère', {
            roomCode,
        ...
        if (matchState.correctAnswersCount >= matchState.highestBid) {
            ...
            logger_1.logger.info('Objectif d\'enchères atteint. Manche gagnée !', { roomCode, team: matchState.answeringTeam });
        ...
        logger_1.logger.info('Réponse incorrecte en Enchère, reprise du chrono', { roomCode });
        resumeAuctionTimer(io, roomCode);
    }
}
/**
 ...
 */
function resumeAuctionTimer(io, roomCode) {
    const matchState = activeMatches.get(roomCode);
    if (!matchState)
        return;
    ...
    (0, TimerManager_1.startTimer)(io, roomCode, matchState.auctionRemainingTime, 'auction', () => handleAuctionTimeout(io, roomCode));
}
/**
...
function handleDiscussionTimeout(io, roomCode) {
    ...
    if (!matchState || !matchState.answeringTeam)
        return;
    matchState.attemptsLeft--;
    logger_1.logger.info('Timeout de réponse en Discussion', { roomCode, attemptsLeft: matchState.attemptsLeft });
    if (matchState.attemptsLeft > 0) {
        io.to(roomCode).emit('match:answerRefused', matchState.attemptsLeft);
        (0, TimerManager_1.startTimer)(io, roomCode, constants_1.GAME_CONSTANTS.DISCUSSION_TIMER, 'discussion', () => handleDiscussionTimeout(io, roomCode));
    }
 ...
 */
function handleAuctionTimeout(io, roomCode) {
    const matchState = activeMatches.get(roomCode);
    if (!matchState || !matchState.answeringTeam)
        return;
    logger_1.logger.info('Timeout global Enchère de 30s atteint', { roomCode, team: matchState.answeringTeam });
    applyLifeLoss(io, roomCode, matchState.answeringTeam);
}
/**
 * Applique la perte de vie à une équipe et gère l'élimination/fin de match.
 */
function applyLifeLoss(io, roomCode, teamColor) {
    const roomState = (0, RoomManager_1.getRoomByCode)(roomCode);
    const matchState = activeMatches.get(roomCode);
    if (!roomState || !matchState)
        return;
    const result = (0, RoomManager_1.loseLife)(roomCode, teamColor);
...
function startNextRound(io, roomCode) {
    ...
    logger_1.logger.info('Nouvelle manche lancée', {
        roomCode,
 ...
 */
function finishMatch(io, roomCode, winner) {
    const roomState = (0, RoomManager_1.getRoomByCode)(roomCode);
    const matchState = activeMatches.get(roomCode);
    if (!roomState || !roomState.bracket || !matchState)
        return;
    ...
    logger_1.logger.info('Match terminé', { roomCode, matchId: matchState.matchId, winner });
 ...
 */
function advanceTournament(io, roomCode) {
    const roomState = (0, RoomManager_1.getRoomByCode)(roomCode);
    if (!roomState || !roomState.bracket)
        return;
    ...
    if (bracket.final && bracket.final.status === 'finished') {
        ...
        logger_1.logger.info('Tournoi terminé ! Champion couronné', { roomCode, champion });
        return;
    }
    // 2. Si les deux demi-finales sont terminées, on génère et démarre la finale
    if (bracket.semiFinal1.status === 'finished' &&
        ...
        logger_1.logger.info('Demi-finales complétées. Finale prête.', {
            roomCode,
    ...
    if (bracket.semiFinal1.status === 'finished' && bracket.semiFinal2.status === 'pending') {
        logger_1.logger.info('SF1 terminée, démarrage automatique de SF2', { roomCode });
        setTimeout(() => {
    ...
    if (bracket.semiFinal2.status === 'finished' && bracket.semiFinal1.status === 'pending') {
        logger_1.logger.info('SF2 terminée, démarrage automatique de SF1', { roomCode });
        setTimeout(() => {
 ...
 */
function handleUseTimeJoker(io, roomCode, socketId) {
    const roomState = (0, RoomManager_1.getRoomByCode)(roomCode);
    const matchState = activeMatches.get(roomCode);
    if (!roomState || !matchState || !matchState.answeringTeam)
        return;
    const player = roomState.players.get(socketId);
    ...
    const newDuration = (0, TimerManager_1.addTime)(roomCode, constants_1.GAME_CONSTANTS.JOKER_TIME_BONUS);
    if (newDuration === null) {
    ...
    logger_1.logger.info('Joker Temps utilisé avec succès', { roomCode, team: team.color, newDuration });
}
/**
 * Joker Indice : Révèle l'indice de la question courante.
 */
function handleUseHintJoker(io, roomCode, socketId) {
    const roomState = (0, RoomManager_1.getRoomByCode)(roomCode);
    const matchState = activeMatches.get(roomCode);
    if (!roomState || !matchState || !matchState.answeringTeam || !matchState.currentQuestion)
        return;
    const player = roomState.players.get(socketId);
    if (!player || !player.teamColor)
        return;
    ...
    io.to(roomCode).emit('lobby:updated', Array.from(roomState.players.values()), Array.from(roomState.teams.values()));
    logger_1.logger.info('Joker Indice utilisé avec succès', { roomCode, team: team.color, hint });
}
